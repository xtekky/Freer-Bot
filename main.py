...
from utils.pycryptojs import CryptoJS
...
response = session.get(
    "https://homedecoratione.com/captcha.php",
    headers={
        "origin": "https://homedecoratione.com/",
        "user-agent": user_agent,
        "x-requested-with": "XMLHttpRequest",
    ...
    params={"_CAPTCHA": "", "t": f"{round(random.random(), 8)} {int(time.time())}"},
)
try:
    image = Image.open(io.BytesIO(response.content))
    image.save("captcha.png")
except:
    sys.exit("wrong captcha")

image.show()
jd = {
    "requests": [
        {
            "image": {"content": str(base64.b64encode(response.content).decode())},
...
data = CryptoJS.format(decrypted)
print(data)
data_param = str(data.decode()).split("zxndnnndje('")[1].split("=")[0]

search_video = session.post(
    url="https://homedecoratione.com/v4.php",
    data={data_param: "tok_free"},
    headers={
        "x-requested-with": "XMLHttpRequest",
        "user-agent": user_agent,
        "origin": "https://homedecoratione.com/",
...
url_data = str(data.decode()).split("zxndnnndje(")[1][1:].split("&'+$")[0]
...
search_video = session.post(
    ...
    data={
        ...
        str(data.decode())
        .split('name="')[1]
        .split('"')[0]: "https://www.tiktok.com/@aarseetimes/video/7123189184977800475",
    },
    headers={
        "x-requested-with": "XMLHttpRequest",
        "user-agent": user_agent,
...
new_params = "=".join(str(data.decode()).split("zxndnnndje('")[2].split("=")[0:3])
print(new_params)

send_views = session.post(
    url = "https://homedecoratione.com/v4.php",
    data = {
        new_params.split('&')[0].split('=')[0]: "tok_auto",
        new_params.split('&')[1].split('=')[0]: "views",
        new_params.split('&')[2]: unquote(str(data.decode()).split("zxndnnndje(\'")[2].split("\"")[0].split("=")[3]) #7123189184977800475
    },
    headers = {
        "x-requested-with": "XMLHttpRequest",
        "user-agent": user_agent,
...
captcha2_raw = unquote(
    re.findall(r'base64,(.+)\\" class=\\"img', unquote(str(data)))[0]
)

try:
    image = Image.open(io.BytesIO(base64.urlsafe_b64decode(captcha2_raw)))
    image.save("captcha.png")
except:
    sys.exit("wrong captcha")
...
jd = {
    "requests": [
        {
            "image": {"content": str(captcha2_raw)},
            "features": [{"type": "TEXT_DETECTION"}],
        }
    ]
}
